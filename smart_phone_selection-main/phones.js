
export const phonesData = [

    {
        name: "OnePlus 13 CN", 
        tags: ["balans", "autonomy", "40", "game", "snap", "fastcharge", "material", "ip", "scan","CN",], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.82 AMOLED - 1440 x 3168<br>Чип: Qualcomm Snapdragon 8 Elite<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 6000 мАч<br>OS: Android 16<br>Вес: 213 г",
        img: "img/OnePlus13.jpg", 
        price: "37 000₽",
        link: "https://nanoreview.net/ru/phone/oneplus-13"
    },

    { 
        name: "Xiaomi 15 CN", 
        tags: ["snap", "40", "fastcharge", "compact", "ip", "material", "scan","camers", "balans","CN"], 
        desc: "Классный компакт, хороший вибро, нормальные камеры и ультразвуковой отпечаток", 
        characteristics: "Дисплей: 6.36 AMOLED - 1200 x 2670<br>Чип: Qualcomm Snapdragon 8 Elite<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 5400 мАч<br>OS: Android 16<br>Вес: 191 г",
        img: "img/xiaomi15.png", 
        price: "33 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-15"
    },  

    {  
        name: "OnePlus 13", 
        tags: ["balans", "autonomy", "50", "game", "snap", "fastcharge", "material", "ip", "scan", "global", "esim","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.82 AMOLED - 1440 x 3168<br>Чип: Qualcomm Snapdragon 8 Elite<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 6000 мАч<br>OS: Android 16<br>Вес: 213 г",
        img: "img/OnePlus13.jpg", 
        price: "42 000₽",
        link: "https://nanoreview.net/ru/phone/oneplus-13"
    },

    {  
        name: "Xiaomi 15", 
        tags: ["snap", "50", "fastcharge", "compact", "ip", "material", "scan", "global", "camers", "balans", "esim","gps"], 
        desc: "Классный компакт, хороший вибро, нормальные камеры и ультразвуковой отпечаток", 
        characteristics: "Дисплей: 6.36 AMOLED - 1200 x 2670<br>Чип: Qualcomm Snapdragon 8 Elite<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 5400 мАч<br>OS: Android 16<br>Вес: 191 г",
        img: "img/xiaomi15.png", 
        price: "40 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-15"
    },

    {  
        name: "Realme gt 8 CN", 
        tags: ["snap", "30", "fastcharge", "game", "ip","balans","autonomy","CN",], 
        desc: " Максимальный FPS, Snapdragon последнего поколения и зарядка до 40% за 15 минут. Лучший выбор для тяжелых игр",
        characteristics: "Дисплей: 6.79 OLED - 1440 x 3136<br>Чип: Qualcomm Snapdragon 8 Elite<br>Камера: 3 (50 MP + 50 MP + 8 MP)<br>Батарея: 7000 мАч<br>OS: Android 16<br>Вес: 209 г", 
        img: "img/realme gt 8.png", 
        price: "30000₽",
        link: "https://nanoreview.net/ru/phone/realme-gt-8-china"

    },    

    { 
        name: "CMF Phone 1", 
        tags: ["10", "fastcharge", "ip","balans","global","gps"], 
        desc: "Съемные панели, аксессуары на винтах и легкая Nothing OS. Самый самобытный бюджетник с акцентом на дизайн", 
        characteristics: "Дисплей: 6.67 AMOLED - 1080 x 2400<br>Чип: MediaTek Dimensity 7300<br>Камера: 2 (50 MP + 2 MP)<br>Батарея: 5000 мАч<br>OS: Android 16<br>Вес: 197 г",
        img: "img/cmf1.png", 
        price: "10000₽",
        link: "https://nanoreview.net/ru/phone/nothing-cmf-phone-1"
    },

    {  
        name: "Xiaomi 15t", 
        tags: ["30", "fastcharge", "ip","balans","camers","global","esim","gps"], 
        desc: "Оптика Leica, влагозащита IP68 и сбалансированный софт. Универсальный выбор для качественных фото и видео",
        characteristics: "Дисплей: 6.83 AMOLED - 1280 x 2772<br>Чип: MediaTek Dimensity 8400 Ultra<br>Камера: 3 (50 MP + 50 MP + 12 MP)<br>Батарея: 5500 мАч<br>OS: Android 16<br>Вес: 194 г", 
        img: "img/xiaomi15t.png", 
        price: "30000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-15t"
    },

    { 
        name: "Realme 15 pro", 
        tags: ["25", "fastcharge", "ip","autonomy","camers","global","esim","snap","gps"], 
        desc: "Оптика Leica, влагозащита IP68 и сбалансированный софт. Универсальный выбор для качественных фото и видео", 
        characteristics: "Дисплей: 6.8 OLED - 1280 x 2800<br>Чип: Qualcomm Snapdragon 7 Gen 4<br>Камера: 2 (50 MP + 50 MP)<br>Батарея: 7000 мАч<br>OS: Android 15<br>Вес: 187 г",
        img: "img/realme15pro.png", 
        price: "20000₽",
        link: "http://nanoreview.net/ru/phone/realme-15-pro-5g"
    },

    {  
        name: "Samsung a56", 
        tags: ["25","ip","global","esim","gps","material","balans"], 
        desc: "Оптика Leica, влагозащита IP68 и сбалансированный софт. Универсальный выбор для качественных фото и видео", 
        characteristics: "Дисплей: 6.7 Super AMOLED - 1080 x 2340<br>Чип: Samsung Exynos 1580<br>Камера: 3 (50 MP + 12 MP + 5 MP)<br>Батарея: 5000 мАч<br>OS: Android 16<br>Вес: 198 г",
        img: "img/a56.jpg", 
        price: "25000₽",
        link: "https://nanoreview.net/ru/phone/samsung-galaxy-a56-5g"
    },

    { 
        name: "Xiaomi 15 pro CN", 
        tags: ["40","ip","CN","material","balans","camers","fastcharge","autonomy","snap","scan"], 
        desc: "Оптика Leica, влагозащита IP68 и сбалансированный софт. Универсальный выбор для качественных фото и видео", 
        characteristics: "Дисплей: 6.73 AMOLED - 1440 x 3200<br>Чип: Qualcomm Snapdragon 8 Elite<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 6100 мАч<br>OS: Android 16<br>Вес: 213 г",
        img: "img/xiaomi15pro.jpeg", 
        price: "40000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-15-pro"
    },
    
    { 
        name: "Xiaomi 15t pro ", 
        tags: ["40","ip","global","material","balans","camers","fastcharge","autonomy","scan"], 
        desc: "Оптика Leica, влагозащита IP68 и сбалансированный софт. Универсальный выбор для качественных фото и видео", 
        characteristics: "Дисплей: 6.83 AMOLED - 1280 x 2772<br>Чип: MediaTek Dimensity 9400 Plus<br>Камера: 3 (50 MP + 50 MP + 12 MP)<br>Батарея: 5500 мАч<br>OS: Android 16<br>Вес: 210 г",
        img: "img/xiaomi15tpro.jpg", 
        price: "40000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-15t-pro"
    },

    { 
        name: "Poco x8 pro", 
        tags: ["25","ip","global","material","game","balans","fastcharge","autonomy","gps"], 
        desc: "Оптика Leica, влагозащита IP68 и сбалансированный софт. Универсальный выбор для качественных фото и видео", 
        characteristics: "Дисплей: 6.59 AMOLED - 1268 x 2756<br>Чип: MediaTek Dimensity 8500 Ultra<br>Камера: 2 (50 MP + 8 MP)<br>Батарея: 6500 мАч<br>OS: Android 16<br>Вес: 201",
        img: "img/pocox8pro.png", 
        price: "25000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-poco-x8-pro"
    },

    { 
        name: "Vivo s50 pro mini CN", 
        tags: ["40","ip","CN","material","camers","balans","fastcharge","autonomy","scan"], 
        desc: "Оптика Leica, влагозащита IP68 и сбалансированный софт. Универсальный выбор для качественных фото и видео", 
        characteristics: "Дисплей: 6.31 AMOLED - 1260 x 2800<br>Чип: Qualcomm Snapdragon 8 Gen 5<br>Камера: 3 (50 MP + 50 MP + 8 MP)<br>Батарея: 6500 мАч<br>OS: Android 16<br>Вес: 191",
        img: "img/vivos50.jpg", 
        price: "40000₽",
        link: "https://nanoreview.net/ru/phone/vivo-s50-pro-mini"
    },

    {
        name: "ROG phone 9 FE", 
        tags: ["balans", "autonomy", "40", "game", "snap", "fastcharge", "material", "ip","global","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1080 x 2400<br>Чип: Qualcomm Snapdragon 8 Gen 3<br>Камера: 3 (50 MP + 13 MP + 5 MP)<br>Батарея: 5500 мАч<br>OS: Android 15<br>Вес: 225 г",
        img: "img/AsusROGPhone9-FE.jpg", 
        price: "40 000₽",
        link: "https://nanoreview.net/ru/phone/asus-rog-phone-9-fe"
    },

    {
        name: "Oneplus 15 CN", 
        tags: ["balans", "autonomy", "45", "game", "snap", "fastcharge", "material", "ip","scan","CN"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1272 x 2772<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 7300 мАч<br>OS: Android 16<br>Вес: 211",
        img: "img/OnePlus15.png", 
        price: "41 000₽",
        link: "https://nanoreview.net/ru/phone/oneplus-15"
    },

    {
        name: "Oneplus 15", 
        tags: ["balans", "autonomy", "45", "game", "snap", "fastcharge", "material", "ip","scan","global","gps","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1272 x 2772<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 7300 мАч<br>OS: Android 16<br>Вес: 211",
        img: "img/OnePlus15.png", 
        price: "53 000₽",
        link: "https://nanoreview.net/ru/phone/oneplus-15"
    },

    {
        name: "Poco f8 ultra", 
        tags: ["balans", "autonomy", "45", "game", "snap", "fastcharge", "material", "ip","scan","global","gps","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.9 AMOLED - 1200 x 2608<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 6500 мАч<br>OS: Android 16<br>Вес: 218",
        img: "img/POCOF8Ultra.jpg", 
        price: "42 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-poco-f8-ultra"
    },
    
    {
        name: "Vivo 300 ultra CN", 
        tags: ["balans", "80", "camers", "snap", "fastcharge", "material", "ip","scan","CN"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.82 AMOLED - 1440 x 3168<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (200 MP + 200 MP + 50 MP)<br>Батарея: 6600 мАч<br>OS: Android 16<br>Вес: 232",
        img: "img/vivo300ultra.jpg", 
        price: "70 000₽",
        link: "https://nanoreview.net/ru/phone/vivo-x300-ultra"
    },

        {
        name: "Vivo 300 ultra", 
        tags: ["balans", "80", "camers", "snap", "fastcharge", "material", "ip","scan","esim","global","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.82 AMOLED - 1440 x 3168<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (200 MP + 200 MP + 50 MP)<br>Батарея: 6600 мАч<br>OS: Android 16<br>Вес: 232",
        img: "img/vivo300ultra.jpg", 
        price: "76 000₽",
        link: "https://nanoreview.net/ru/phone/vivo-x300-ultra"
    },

    {
        name: "Xiaomi 17", 
        tags: ["snap", "fastcharge", "compact", "ip", "material", "scan", "global", "camers", "balans", "esim","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.3 AMOLED - 1220 x 2656<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 6330 мАч<br>OS: Android 16<br>Вес: 191",
        img: "img/Xiaomi 17.jpg", 
        price: "52 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-17"
    },

        {
        name: "Xiaomi 17 CN", 
        tags: ["snap", "fastcharge", "compact", "ip", "material", "scan", "CN", "camers", "balans"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.3 AMOLED - 1220 x 2656<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 6330 мАч<br>OS: Android 16<br>Вес: 191",
        img: "img/Xiaomi 17.jpg", 
        price: "48 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-17"
    },

    { 
        name: "Oppo x8 ultra CN", 
        tags: ["snap", "65","ip","CN","material","camers","balans","fastcharge","autonomy","scan"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.82 AMOLED - 1440 x 3168<br>Чип: Qualcomm Snapdragon 8 Elite<br>Камера: 4 (50 MP + 50 MP + 50 MP + 50 MP)<br>Батарея: 6100 мАч<br>OS: Android 16<br>Вес: 226",
        img: "img/x8u.jpg", 
        price: "65 000₽",
        link: "https://nanoreview.net/ru/phone/oppo-find-x8-ultra"
    },  

    {
 
        name: "Realme neo 8", 
        tags: ["ip","global","game","balans","fastcharge","autonomy","esim","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1272 x 2772<br>Чип: Qualcomm Snapdragon 8 Gen 5<br>Камера: 3 (50 MP + 50 MP + 8 MP)<br>Батарея: 8000 мАч<br>OS: Android 16<br>Вес: 215 г",
        img: "img/0.22801700_1770726748_realme-neo-8-5g-rmx8899-dual-sim-256gb-white-12gb-ram-china-version.jpg", 
        price: "28 000₽",
        link: "https://nanoreview.net/ru/phone/realme-neo-8"
    },
    {
 
        name: "Realme gt 7", 
        tags: ["ip","global","game","balans","fastcharge","autonomy","esim","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1264 x 2780<br>Чип: MediaTek Dimensity 9400e<br>Камера: 3 (50 MP + 50 MP + 8 MP)<br>Батарея: 7000 мАч<br>OS: Android 15<br>Вес: 206",
        img: "img/realmegt7.jpg", 
        price: "27 000₽",
        link: "https://nanoreview.net/ru/phone/realme-gt-7"
    },
    { 
        name: "Oppo x9 pro", 
        tags: ["snap", "65","ip","global","material","camers","balans","fastcharge","autonomy","scan","esim","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1264 x 2780<br>Чип: MediaTek Dimensity 9500<br>Камера: 3 (50 MP + 200 MP + 50 MP)<br>Батарея: 7500 мАч<br>OS: Android 16<br>Вес: 224",
        img: "img/oppox9pro.jpg", 
        price: "65 000₽",
        link: "https://nanoreview.net/ru/phone/oppo-find-x9-pro"
    },

    { 
        name: "Oppo x9 pro CN", 
        tags: ["snap", "65","ip","CN","material","camers","balans","fastcharge","autonomy","scan"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1264 x 2780<br>Чип: MediaTek Dimensity 9500<br>Камера: 3 (50 MP + 200 MP + 50 MP)<br>Батарея: 7500 мАч<br>OS: Android 16<br>Вес: 224",
        img: "img/oppox9pro.jpg", 
        price: "48 000₽",
        link: "https://nanoreview.net/ru/phone/oppo-find-x9-pro"
    },

    { 
        name: "Poco f8 pro", 
        tags: ["balans", "autonomy", "game", "snap", "fastcharge", "material", "ip","scan","global","gps","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.6  AMOLED - 1156 x 2510<br>Чип: Qualcomm Snapdragon 8 Elite<br>Камера: 3 (50 MP + 50 MP + 8 MP)<br>Батарея: 6210 мАч<br>OS: Android 16<br>Вес: 199",
        img: "img/pocof8pro.jpg", 
        price: "32 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-poco-f8-pro"
    },

    { 
        name: "Nubia z80 ultra", 
        tags: ["snap","ip","global","material","camers","balans","fastcharge","autonomy","scan","esim","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.85 AMOLED - 1216 x 2688<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 64 MP + 50 MP)<br>Батарея: 7200 мАч<br>OS: Android 16<br>Вес: 227",
        img: "img/nubiaz80u.jpg", 
        price: "50 000₽",
        link: "https://nanoreview.net/ru/phone/zte-nubia-z80-ultra"
    }, 
    
    { 
        name: "Oneplus nord 5", 
        tags: ["balans", "autonomy","game", "snap", "fastcharge","ip","global","gps","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.83 AMOLED - 1272 x 2800<br>Чип: Qualcomm Snapdragon 8s Gen 3<br>Камера: 2 (50 MP + 8 MP)<br>Батарея: 5200 мАч<br>OS: Android 15<br>Вес: 211",
        img: "img/nord5.jpg", 
        price: "25 000₽",
        link: "https://nanoreview.net/ru/phone/oneplus-nord-5"
    }, 

    { 
        name: "Motorola moto g56", 
        tags: ["balans","fastcharge","ip","global","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.72 IPS LCD - 1080 x 2400<br>Чип: MediaTek Dimensity 7060<br>Камера: 2 (50 MP + 8 MP)<br>Батарея: 5200 мАч<br>OS: Android 15<br>Вес: 200",
        img: "img/motog56.jpg", 
        price: "15 000₽",
        link: "https://nanoreview.net/ru/phone/motorola-moto-g56-5g"
    },

    { 
        name: "Iqoo z11 turbo", 
        tags: ["balans","fastcharge","ip","CN","game","snap"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.59 AMOLED - 1260 x 2750<br>Чип: Qualcomm Snapdragon 8 Gen 5<br>Камера: 2 (200 MP + 8 MP)<br>Батарея: 7600 мАч<br>OS: Android 16<br>Вес: 202",
        img: "img/z11turbo.jpg", 
        price: "23 000₽",
        link: "https://nanoreview.net/ru/phone/vivo-iqoo-z11-turbo"
    },

    { 
        name: "Nothing phone 4a", 
        tags: ["balans","fastcharge","global","game","snap","gps","camers"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1224 x 2720<br>Чип: Qualcomm Snapdragon 7s Gen 4<br>Камера: 3 (50 MP + 50 MP + 8 MP)<br>Батарея: 5080 мАч<br>OS: Android 16<br>Вес: 205",
        img: "img/nphon4a.jpg", 
        price: "25 000₽",
        link: "https://nanoreview.net/ru/phone/nothing-phone-4a"
    },    
    
    { 
        name: "Vivo x300 CN", 
        tags: ["ip","CN","material","camers","balans","fastcharge","autonomy","scan","compact"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.31 AMOLED - 1216 x 2640<br>Чип: MediaTek Dimensity 9500<br>Камера: 3 (200 MP + 50 MP + 50 MP)<br>Батарея: 6040 мАч<br>OS: Android 16<br>Вес: 190",
        img: "img/vivox300.jpg", 
        price: "41 000₽",
        link: "https://nanoreview.net/ru/phone/vivo-x300"
    },
    
    { 
        name: "Vivo x300  RU", 
        tags: ["ip","global","material","camers","balans","fastcharge","autonomy","scan","compact","esim","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.31 AMOLED - 1216 x 2640<br>Чип: MediaTek Dimensity 9500<br>Камера: 3 (200 MP + 50 MP + 50 MP)<br>Батарея: 6040 мАч<br>OS: Android 16<br>Вес: 190",
        img: "img/vivox300.jpg", 
        price: "45 000₽",
        link: "https://nanoreview.net/ru/phone/vivo-x300"
    },

    { 
        name: "Meizu lucky 08 CN", 
        tags: ["fastcharge","snap","ip","balans",  "CN"], 
        desc: "Съемные панели, аксессуары на винтах и легкая Nothing OS. Самый самобытный бюджетник с акцентом на дизайн", 
        characteristics: "Дисплей: 6.78 LTPO OLED — 1264 x 2780<br>Чип: Qualcomm Snapdragon 7s Gen 2<br>Камера: 2 (108 MP + 2 MP)<br>Батарея: 6000 мАч<br>OS: Android 14 Flyme OS<br>Вес: 202 г",
        img: "img/meizu lucky 08.jpg", 
        price: "12000₽",
        link: "https://nanoreview.net/ru/phone/nothing-cmf-phone-1"
    },
    
    { 
        name: "Realme 15t", 
        tags: ["fastcharge","ip","balans","global","gps"], 
        desc: "Съемные панели, аксессуары на винтах и легкая Nothing OS. Самый самобытный бюджетник с акцентом на дизайн", 
        characteristics: "Дисплей: 6.57 AMOLED - 1080 x 2372<br>Чип: MediaTek Dimensity 6400 Max<br>Камера: 2 (50 MP + 2 MP)<br>Батарея: 7000 мАч<br>OS: Android 15<br>Вес: 181 г",
        img: "img/realme15t.jpg", 
        price: "12000₽",
        link: "https://nanoreview.net/ru/phone/realme-15t"
    },

        { 
        name: "Oppo x9 ultra CN", 
        tags: ["snap", "90","ip","CN","material","camers","balans","fastcharge","autonomy","scan","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.82 AMOLED - 1440 x 3168<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 4 (200 MP + 200 MP + 50 MP + 50 MP)<br>Батарея: 7050 мАч<br>OS: Android 16<br>Вес: 235<br>esim: Только в версии на 1тб",
        img: "img/ч9г.jpg", 
        price: "91 000₽",
        link: "https://nanoreview.net/ru/phone/oppo-find-x9-ultra"
    }, 



    { 
        name: "OnePlus Turbo 6V CN", 
        tags: ["balans","fastcharge","ip","CN","game","snap"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1272 x 2772<br>Чип: Qualcomm Snapdragon 7s Gen 4<br>Камера: 2 (50 MP)<br>Батарея: 9000 мАч<br>OS: Android 16<br>Вес: 215 г",
        img: "img/OnePlusTurbo6V.jpg", 
        price: "23 000₽",
        link: "https://nanoreview.net/en/phone/oneplus-turbo-6v"
    },

    { 
        name: "Poco m8s", 
        tags: ["balans","fastcharge","ip","global","game","snap","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.9 AMOLED - 1080 x 2340<br>Чип: Qualcomm Snapdragon 6s Gen 3<br>Камера: 2 (50 MP)<br>Батарея: 7000 мАч<br>OS: Android 15<br>Вес: 217",
        img: "img/pocom8s.jpg", 
        price: "10 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-poco-m8s-5g"
    },

    { 
        name: "Poco m8", 
        tags: ["balans","fastcharge","ip","global","game","snap","gps"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.77 AMOLED - 1080 x 2340<br>Чип: Qualcomm Snapdragon 6 Gen 3<br>Камера: 2 (50 MP)<br>Батарея: 5520 мАч<br>OS: Android 15<br>Вес: 178 г",
        img: "img/m8.jpg", 
        price: "12 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-poco-m8-5g"
    },

    { 
        name: "Honor magic 8 pro", 
        tags: ["snap","ip","global","material","camers","balans","fastcharge","autonomy","scan","gps","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.71 OLED - 1256 x 2808<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 200 MP + 50 MP)<br>Батарея: 7200 мАч<br>OS: Android 16<br>Вес: 219 г",
        img: "img/magic8pro.jpg", 
        price: "70 000₽",
        link: "https://nanoreview.net/ru/phone/honor-magic-8-pro"
    },

    { 
        name: "Honor magic8 pro CN", 
        tags: ["snap","ip","CN","material","camers","balans","fastcharge","autonomy","scan"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.71 OLED - 1256 x 2808<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 200 MP + 50 MP)<br>Батарея: 7200 мАч<br>OS: Android 16<br>Вес: 219 г",
        img: "img/magic8pro.jpg", 
        price: "64 000₽",
        link: "https://nanoreview.net/ru/phone/honor-magic-8-pro"
    },

    { 
        name: "Vivo x300 pro CN", 
        tags: ["snap","ip","CN","material","camers","balans","fastcharge","autonomy","scan"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1260 x 2800<br>Чип: MediaTek Dimensity 9500<br>Камера: 3 (50 MP + 200 MP + 50 MP)<br>Батарея: 6510 мАч<br>OS: Android 16<br>Вес: 226 г",
        img: "img/vivox300pro.jpg", 
        price: "55 000₽",
        link: "https://nanoreview.net/ru/phone/vivo-x300-pro"
    },

    { 
        name: "Vivo x300 pro", 
        tags: ["snap","ip","global","material","camers","balans","fastcharge","autonomy","scan","gps","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.78 AMOLED - 1260 x 2800<br>Чип: MediaTek Dimensity 9500<br>Камера: 3 (50 MP + 200 MP + 50 MP)<br>Батарея: 6510 мАч<br>OS: Android 16<br>Вес: 226 г",
        img: "img/vivox300pro.jpg", 
        price: "63 000₽",
        link: "https://nanoreview.net/ru/phone/vivo-x300-pro"
    },
    
    { 
        name: "Oneplus 15r", 
        tags: ["balans","fastcharge","ip","global","game","snap","esim","gps","material"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.83 AMOLED - 1272 x 2800<br>Чип: Qualcomm Snapdragon 8 Gen 5<br>Камера: 2 (50 MP + 8 MP)<br>Батарея: 7400 мАч<br>OS: Android 16<br>Вес: 214 г",
        img: "img/1+15r.jpg", 
        price: "30 000₽",
        link: "https://nanoreview.net/ru/phone/oneplus-15r"
    },

    { 
        name: "Samsung s25 ultra", 
        tags: ["snap","ip","global","material","camers","balans","autonomy","scan","gps","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.9 Dynamic AMOLED - 1440 x 3120<br>Чип: Qualcomm Snapdragon 8 Elite<br>Камера: 4 (200 MP + 50 MP + 10 MP + 50 MP)<br>Батарея: 5000 мАч<br>OS: Android 16<br>Вес: 218 г",
        img: "img/SamsungGalaxyS25Ultra.jpg", 
        price: "66 000₽",
        link: "https://nanoreview.net/ru/phone/samsung-galaxy-s25-ultra"
    },

    { 
        name: "Xiaomi 17 ultra", 
        tags: ["snap", "90","ip","global","material","camers","balans","fastcharge","autonomy","scan","gps","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.9 AMOLED - 1200 x 2608<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 200 MP + 50 MP)<br>Батарея: 6000 мАч<br>OS: Android 16<br>Вес: 218.4 г",
        img: "img/Xiaomi17Ultra.jpg", 
        price: "90 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-17-ultra"
    },

    { 
        name: "Xiaomi 17 ultra CN", 
        tags: ["snap", "90","ip","CN","material","camers","balans","fastcharge","autonomy","scan"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.9 AMOLED - 1200 x 2608<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 200 MP + 50 MP)<br>Батарея: 6000 мАч<br>OS: Android 16<br>Вес: 218.4 г",
        img: "img/Xiaomi17Ultra.jpg", 
        price: "80 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-17-ultra"
    },

    { 
        name: "Poco x8 pro max", 
        tags: ["balans", "autonomy","game", "snap", "fastcharge","ip","global","gps","esim","material","scan"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "исплей: 6.83 AMOLED - 1280 x 2772<br>Чип: MediaTek Dimensity 9500s<br>Камера: 2 (50 MP + 8 MP)<br>Батарея: 8500 мАч<br>OS: Android 16<br>Вес: 218 г",
        img: "img/pocox8pm.jpg", 
        price: "28 000₽",
        link: "https://nanoreview.net/ru/phone/xiaomi-poco-x8-pro-max"
    }, 

    { 
        name: "Realme 14 pro plus", 
        tags: ["balans","fastcharge","ip","global","gps","camers"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.83 OLED - 1272 x 2800<br>Чип: Qualcomm Snapdragon 7s Gen 3<br>Камера: 3 (50 MP + 50 MP + 8 MP)<br>Батарея: 6000 мАч<br>OS: Android 16<br>Вес: 194 г",
        img: "img/realme14proplus.jpg", 
        price: "26 000₽",
        link: "https://nanoreview.net/ru/phone/realme-14-pro-plus"
    },    

    { 
        name: "Iqoo 15 ultra CN", 
        tags: ["balans","fastcharge","ip","CN","game","snap","material","autonomy"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.85 AMOLED - 1440 x 3168<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 50 MP + 50 MP)<br>Батарея: 7400 мАч<br>OS: Android 16<br>Вес: 227 г",
        img: "img/vivoiqoo15ultra.jpg", 
        price: "64 000₽",
        link: "https://nanoreview.net/en/phone/vivo-iqoo-15-ultra"
    },

    { 
        name: "Honor magic 7 pro", 
        tags: ["snap", "65","ip","global","material","camers","balans","fastcharge","autonomy","scan","gps","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.8 OLED - 1280 x 2800<br>Чип: Qualcomm Snapdragon 8 Elite<br>Камера: 3 (50 MP + 200 MP + 50 MP)<br>Батарея: 5850 мАч<br>OS: Android 16<br>Вес: 223 г",
        img: "img/honor1.jpg", 
        price: "51 000₽",
        link: "https://nanoreview.net/ru/phone/honor-magic-7-pro"
    },  

    {  
        name: "Vivo v70", 
        tags: ["30", "fastcharge", "ip","balans","camers","global","esim","gps","snap"], 
        desc: "Оптика Leica, влагозащита IP68 и сбалансированный софт. Универсальный выбор для качественных фото и видео",
        characteristics: "Дисплей: 6.59 AMOLED - 1260 x 2750<br>Чип: Qualcomm Snapdragon 7 Gen 4<br>Камера: 3 (50 MP + 50 MP + 8 MP)<br>Батарея: 6500 мАч<br>OS: Android 16<br>Вес: 187 г", 
        img: "img/59622.700.jpg", 
        price: "24000₽",
        link: "https://nanoreview.net/ru/phone/vivo-v70"
    },

    { 
        name: "Oppo find x9 ultra", 
        tags: ["snap", "90","ip","global","material","camers","balans","fastcharge","autonomy","scan","gps","esim"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.82 AMOLED - 1440 x 3168<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 4 (200 MP + 200 MP + 50 MP + 50 MP)<br>Батарея: 7050 мАч<br>OS: Android 16<br>Вес: 235 г",
        img: "img/q93_d9405b0e08acb169d2643eab218c77a2425d105681066b81886648a354dca7ea.png", 
        price: "90 000₽",
        link: "https://nanoreview.net/ru/phone/oppo-find-x9-ultra",
        data: "Апрель 2026 года"
    },

        {  
        name: "Vivo v70", 
        tags: ["30", "fastcharge", "ip","balans","camers","global","esim","gps","snap"], 
        desc: "Оптика Leica, влагозащита IP68 и сбалансированный софт. Универсальный выбор для качественных фото и видео",
        characteristics: "Дисплей: 6.32 AMOLED - 1216 x 2640<br>Чип: MediaTek Dimensity 8450<br>Камера: 3 (200 MP + 50 MP + 50 MP)<br>Батарея: 6200 мАч<br>OS: Android 16<br>Вес: 187 г", 
        img: "img/OppoReno15Pro.jpg", 
        price: "32000₽",
        link: "https://nanoreview.net/ru/phone/oppo-reno-15-pro"
    },    

    {  
        name: "Realme gt 8 pro", 
        tags: ["balans", "autonomy","game", "snap", "fastcharge", "material", "ip", "scan", "CN"], 
        desc: "Прекрасный баланс, супер ОС, лучший в своем бюджете", 
        characteristics: "Дисплей: 6.79 OLED - 1440 x 3136<br>Чип: Qualcomm Snapdragon 8 Elite Gen 5<br>Камера: 3 (50 MP + 200 MP + 50 MP)<br>Батарея: 7000 мАч<br>OS: Android 16<br>Вес: 214 г",
        img: "img/RealmeGT8PRO.jpg", 
        price: "48 000₽",
        link: "https://nanoreview.net/ru/phone/realme-gt-8-pro-china"
    },




































];

export default phonesData;
