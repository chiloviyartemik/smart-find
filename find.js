...
const tagDescriptions = {
    "snap": "Процессоры только от Qualcomm.",
    ...
    "ip": "Защита от воды и пыли по стандарту IP.",
    "esim": "Поддержка виртуальной сим-карты.",
    ...
    buttons.forEach(btn => {
        ...
        if (description) {
            ...
            icon.addEventListener('click', (e) => {
                ...
                // 2. Жестко удаляем через 5 секунд, что бы ни происходило
                setTimeout(() => {
...
let bnavf = document.querySelector(".bnavf");
...
function burger() {
    ...
    if (burg.classList.contains("active")) {
        ...
        if (bnavf) bnavf.classList.remove("active");
...
let allsort = document.querySelectorAll(".sortimp");
let sortchange = "DOWN"; // up  от дешевых к дорогим down  от дорогих к дешевым

// Перебираем все кнопки сортировки и вешаем на них событие клика
allsort.forEach(button => {
    button.addEventListener("click", function() {
        // Получаем направление ("up" или "down") из атрибута name нажатой кнопки
        sortchange = this.getAttribute("id"); 
        
        // Запускаем поиск заново, чтобы товары пересортировались
        find(); 
    });
});
...
function find() {
    ...
    // Вспомогательная функция для получения чистого числа из строки цены
    const getPriceNumber = (phoneObj) => Number(phoneObj.price.replace(/\D/g, ""));

    if(sortchange == "UP"){
           bace.sort((a, b) => {
        return getPriceNumber(a) - getPriceNumber(b);// Если нужно от дорогих к дешевым, поменяйте на: return getPriceNumber(b) - getPriceNumber(a);
    }) 
    };
    if(sortchange == "DOWN"){
           bace.sort((a, b) => {
    ...
    // 2. ЦИКЛ ВЫВОДА (уже отсортированного массива)
    for (let i = 0; i < bace.length; i++) {
        let phone = bace[i];
        let phonePrice = getPriceNumber(phone); 
                ...
                <div class="phone_card">
                    ...
                    <div class="phone_info">
                        ...
                        <span class="price">${phone.price}</span>
                              <span class="data">${phone.data}</span>
                        <div class="card_filtrs">
                            ${tagsHtml}
                        </div>
                        <a href="${phone.link}" class="more_info_btn2">Подробнее</a>
...
allbutton.addEventListener("click", function(){
    ...
    if (condition2 == false){
        allbutton.innerHTML = "Показать кнопки";
        allFilterBtns[i].style.display = "none";
        select.style.display = "none";        
    }
    else{        
        allbutton.innerHTML = "Убрать кнопки";
